...
rebuild-docs:
	source $(VENV) && sphinx-apidoc . --full -o docs -H 'safetar' -A 'Artur Barseghyan <artur.barseghyan@gmail.com>' -f -d 20
...
clean-dev:
	...
	rm -rf dist/ src/safetar.egg-info/ .cache/ .mypy_cache/ .ruff_cache/
...
update-version:
	...
	@if [ "$(UNAME_S)" = "Darwin" ]; then \
		...
		gsed -i 's/__version__ = "[0-9.]\+"/__version__ = "$(VERSION)"/' src/safetar/__init__.py; \
	else \
		sed -i 's/^version = "[0-9.]\+"/version = "$(VERSION)"/' pyproject.toml; \
		sed -i 's/__version__ = "[0-9.]\+"/__version__ = "$(VERSION)"/' src/safetar/__init__.py; \
	fi
