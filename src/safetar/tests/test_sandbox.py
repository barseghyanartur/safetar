...
class TestSymlinkPolicy:
    ...
    def test_resolve_internal_allows_safe_symlink(
        ...
        # RESOLVE_INTERNAL creates a regular file containing the target path as bytes
        assert (dest / "internal_link.txt").is_file()
        assert not (dest / "internal_link.txt").is_symlink()
        assert (dest / "internal_link.txt").read_text() == "target.txt"
    ...
    def test_resolve_internal_accepts_legitimate_nested_target(self, tmp_path):
        ...
        assert not (dest / "link_to_nested").is_symlink()
        assert (dest / "link_to_nested").read_text() == "subdir/target.txt"
